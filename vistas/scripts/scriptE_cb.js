...
function initMap() {
    ...
    var markers = [
        ...
        $id = $_GET['id'];
                ...
                $result = $conexion->query("SELECT nombre,lat,lon,icon FROM registros_material_c"); 
                    if($result->num_rows > 0){ 
                                while($row = $result->fetch_assoc()){ 
                                    echo '["'.$row['nombre'].'", '.$row['lat'].', '.$row['lon'].', "'.$row['icon'].'"],'; 
                 } 
             } 
        ?>
    ];
...
$result2 = $conexion->query("SELECT nombre,direccion,curp,telefono,id,foto FROM registros_material_c where servicio = 'clara'"); 
 ...
 if($result2->num_rows > 0){ 
            while($row = $result2->fetch_assoc()){ ?>
                ...
                '<img src="img/4taT.png" width="20%">'+
                
                '<h5 style="color:white;">COLABORADOR: </h5><h4 style="color:white;"><?php echo $row['nombre']; ?></h4>' +
                
                '<h5 style="color:white;">FECHA: </h5><h4 style="color:white;"><?php echo $row['curp']; ?></h4>' + 
                
                
                '<h5 style="color:white;">HORA: </h5><h4 style="color:white;"><?php echo $row['direccion']; ?></h4>' +                 
